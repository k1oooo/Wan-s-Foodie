...
export default function MenuClient({ initialItems }: { initialItems: MenuItem[] }) {
  ...
  const grouped = useMemo(() => {
    return CATEGORIES.map((category) => ({
      category,
      items: items.filter((i) => i.category === category).sort((a, b) => a.sort_order - b.sort_order),
    }));
  }, [items]);
  ...
  async function saveDraft() {
    ...
    if (draft.id) {
      ...
      if (!error) {
        setItems((prev) => prev.map((i) => (i.id === draft.id ? { ...i, ...draft, id: draft.id! } : i)));
      ...
      if (!error && data) {
        ...
        toast.success(`${draft.name} added to the menu`);
      } else {
        toast.error("Couldn't add item", { description: "Please try again." });
      }
    }
  ...
  async function toggleAvailability(item: MenuItem) {
    ...
    if (error) {
      setItems((prev) => prev.map((i) => (i.id === item.id ? { ...i, is_available: item.is_available } : i))); // revert
      toast.error("Couldn't update availability", { description: "Please try again." });
    } else {
      router.refresh();
    }
  }
  ...
  async function updateStock(item: MenuItem, stock_boxes: number) {
    ...
    setItems((prev) => prev.map((i) => (i.id === item.id ? { ...i, stock_boxes: clamped } : i)));

    const supabase = createClient();
    const { error } = await supabase.from("menu_items").update({ stock_boxes: clamped }).eq("id", item.id);
    if (error) {
      setItems((prev) => prev.map((i) => (i.id === item.id ? { ...i, stock_boxes: item.stock_boxes } : i))); // revert
      ...
      {grouped.map(({ category, items: categoryItems }) => (
        <div key={category} className="rounded-xl border border-slate-200 bg-white">
          <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3 sm:px-5">
            <span className={`h-2.5 w-2.5 rounded-full ${CATEGORY_STYLE[category].dot}`} />
            <h2 className="text-sm font-semibold text-slate-900">{CATEGORY_STYLE[category].label}</h2>
              ...
              <tbody>
                {categoryItems.map((item) => (
                  <tr key={item.id} className="border-b border-slate-50 last:border-0">
                    <td className="px-4 py-3 sm:px-5">
                      <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-slate-100">
                          {item.image_url ? (
                            <Image src={item.image_url} alt={item.name} width={36} height={36} className="h-full w-full object-cover" />
                          ) : (
                    ...
                    <td className="whitespace-nowrap px-4 py-3 tabular-nums text-slate-700 sm:px-5">{formatRM(item.price_per_box)}</td>
                    <td className="px-4 py-3 sm:px-5">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => updateStock(item, Number(e.target.value))}
                      ...
                      >
                        <span
                          className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${
                            item.is_available ? "translate-x-4.5" : "translate-x-1"
                          }`}
                        />
                      </button>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 p-0 sm:items-center sm:p-4">
          <div className="w-full max-w-md rounded-t-2xl bg-white p-5 sm:rounded-2xl">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-semibold text-slate-900">{draft.id ? "Edit item" : "Add item"}</h3>
              <button onClick={() => setDraft(null)} className="rounded-md p-1 text-slate-400 hover:bg-slate-100">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="mt-4 space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-medium text-slate-600">Price / box (RM)</label>
                  <input
                    type="number"
                    min={0}
                    step={0.5}
                    value={draft.price_per_box}
                    onChange={(e) => setDraft({ ...draft, price_per_box: Number(e.target.value) })}
                    className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200"
                  />
                </div>
              </div>
              <div>
                <label className="text-xs font-medium text-slate-600">Stock (boxes)</label>
                <input
                  type="number"
                  min={0}
                  value={draft.stock_boxes}
                  onChange={(e) => setDraft({ ...draft, stock_boxes: Number(e.target.value) })}
                  className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200"
                />
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 p-0 sm:items-center sm:p-4">
          <div className="w-full max-w-sm rounded-t-2xl bg-white p-5 sm:rounded-2xl">
            <h3 className="text-base font-semibold text-slate-900">Delete {deleteTarget.name}?</h3>
            <p className="mt-1.5 text-sm text-slate-500">
              This removes it from the menu. Past orders that included this item are not affected.
            </p>
            <div className="mt-5 flex justify-end gap-2">
              <button
