...
import { DollarSign, ShoppingBag, Clock, Users, ArrowUpRight, Phone, FileDown, Loader2 } from "lucide-react";
import StatCard from "../_components/StatCard";
import { CATEGORY_STYLE, ORDER_STATUS_STYLE, PAYMENT_STATUS_STYLE } from "../_lib/category-style";
...
export default function DashboardClient({ monthOrders, recentOrders, menuItems, regularCustomers }: DashboardClientProps) {
  ...
  const boxesByItem = new Map<string, { name: string; category: Category; boxes: number; revenue: number }>();
  paidOrders.flatMap((o) => o.items).forEach((item) => {
    const menuItem = menuItems.find((m) => m.id === item.menu_item_id);
    const key = item.item_name;
    const current = boxesByItem.get(key) ?? { name: item.item_name, category: menuItem?.category ?? "chicken", boxes: 0, revenue: 0 };
    current.boxes += item.quantity_boxes;
    current.revenue += item.subtotal;
    boxesByItem.set(key, current);
  });
  const topItems = [...boxesByItem.values()].sort((a, b) => b.revenue - a.revenue).slice(0, 5);
  ...
  async function handleDownloadReport() {
    ...
    try {
      ...
      // @ts-expect-error - lastAutoTable is attached to the doc instance after each autoTable() call
      let nextY = doc.lastAutoTable.finalY + 10;

      doc.setFontSize(12);
      doc.setTextColor(15, 23, 42);
      doc.text("Top Selling Items", 14, nextY);

      autoTable(doc, {
        startY: nextY + 4,
        head: [["Item", "Boxes Sold", "Revenue"]],
        body: topItems.map((i) => [i.name, String(i.boxes), formatRM(i.revenue)]),
        theme: "grid",
        headStyles: { fillColor: [15, 23, 42] },
      });
      ...
      doc.setFontSize(12);
      doc.text("Regular Customers", 14, nextY);

      autoTable(doc, {
        startY: nextY + 4,
        head: [["Customer", "Phone", "Paid Orders", "Lifetime Spend", "Last Order"]],
        body: regularCustomers.map((c) => [
          c.customer_name,
          c.customer_phone,
      ...
      const filename = `wans-foodies-sales-report-${new Date().toISOString().slice(0, 7)}.pdf`;
      doc.save(filename);
      toast.success("Sales report downloaded");
    } catch {
      toast.error("Couldn't generate the report", { description: "Please try again." });
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Revenue (paid orders)" value={formatRM(totalRevenue)} icon={DollarSign} hint={monthLabel} />
        <StatCard label="Total orders" value={String(monthOrders.length)} icon={ShoppingBag} hint={monthLabel} />
        <StatCard label="Pending orders" value={String(pendingCount)} icon={Clock} hint="Need confirmation" tone={pendingCount > 0 ? "warning" : "default"} />
        <StatCard label="Regular customers" value={String(regularCustomers.length)} icon={Users} hint="Lifetime, 2+ paid orders" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5 lg:col-span-2">
          <div className="flex items-baseline justify-between">
            <h2 className="text-sm font-semibold text-slate-900">Top selling items</h2>
            <span className="text-xs text-slate-400">{monthLabel}</span>
          </div>
          {topItems.length === 0 ? (
            <p className="mt-4 text-sm text-slate-500">No paid orders yet this month.</p>
          ) : (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          {lowStockItems.length === 0 ? (
            <p className="mt-4 text-sm text-slate-500">All items are well stocked.</p>
          ) : (
            <ul className="mt-4 space-y-3">
              {lowStockItems.map((item) => {
                ...
                return (
                  <li key={item.id} className="flex items-center justify-between text-sm">
                    ...
                    <span className={`font-medium tabular-nums ${item.stock_boxes === 0 ? "text-red-600" : "text-amber-600"}`}>
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        <div className="flex items-center justify-between border-b border-slate-200 p-4 sm:p-5">
          ...
          <Link href="/admin/orders" className="inline-flex items-center gap-1 text-xs font-medium text-slate-600 hover:text-slate-900">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {recentOrders.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-sm text-slate-400 sm:px-5">
                    No orders yet.
                  </td>
                </tr>
              ) : (
                recentOrders.map((order) => (
                  <tr key={order.id} className="border-b border-slate-50 last:border-0">
                    <td className="whitespace-nowrap px-4 py-3 font-medium text-slate-900 sm:px-5">{order.order_number}</td>
                    <td className="whitespace-nowrap px-4 py-3 text-slate-600 sm:px-5">{order.customer_name}</td>
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        <div className="border-b border-slate-200 p-4 sm:p-5">
          <h2 className="text-sm font-semibold text-slate-900">Regular customers</h2>
          <p className="mt-0.5 text-xs text-slate-500">Customers with more than one paid order, all time</p>
        </div>
        {regularCustomers.length === 0 ? (
          <p className="p-6 text-center text-sm text-slate-500">No repeat customers yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {regularCustomers
                  ...
                  .map((c) => (
                    <tr key={c.customer_phone} className="border-b border-slate-50 last:border-0">
                      <td className="px-4 py-3 sm:px-5">
                        <p className="font-medium text-slate-900">{c.customer_name}</p>
                        <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-500">
                          <Phone className="h-3 w-3" /> {c.customer_phone}
                        </p>
                      </td>
                      <td className="whitespace-nowrap px-4 py-3 tabular-nums text-slate-700 sm:px-5">{c.total_orders}</td>
                      <td className="whitespace-nowrap px-4 py-3 font-medium tabular-nums text-slate-900 sm:px-5">
                        {formatRM(c.lifetime_spend)}
                      </td>
                      <td className="whitespace-nowrap px-4 py-3 text-slate-500 sm:px-5">{formatDate(c.last_order_at)}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
