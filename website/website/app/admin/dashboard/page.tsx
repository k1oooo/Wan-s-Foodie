...
export default async function DashboardPage() {
  ...
  const [monthOrdersRes, recentOrdersRes, menuItemsRes, regularsRes] = await Promise.all([
    supabase
      .from("orders")
      .select("*, order_items(*)")
  ...
  const monthOrders: Order[] = (monthOrdersRes.data ?? []).map((o) => ({ ...o, items: o.order_items ?? [] }));
  const recentOrders: Order[] = (recentOrdersRes.data ?? []).map((o) => ({ ...o, items: o.order_items ?? [] }));
